...
'<#ins data-ins="$1"><#/ins><#').replace(/(<del data-del=")([\s\S]+?)(".*?)></g,"$1$2$3>$2<").replace(/<del>([\s\S]*?)(?:\x20*<.*?( data-ins="(.*?)").*?(<#\/ins>)+)?</g,function(a,b,c,d){return'<del data-del="'+b.replace(/\n/,"\\n")+'" data-ins="'+("*"!=b?d?d:".35":".25")+'">'+("*"!=b?b:"")+"<"}).replace(/(?!<ins)<(\w+)(.*?)>/g,"<$1$2></$1>").replace(/(<ins>)([\s\S]+?)(<\/ins>)/g,function(a,b,c,d){return b+c.replace(/<(.*?)><\/.*?>/g,"<$1>")+d}).replace(/<#/g,"<").match(/<ins>[\s\S]+?<\/ins>|<\/.*?>|<[\s\S]+?<\/.*?>|<[\s\S]+?>|&[#x]?[a-z0-9]+;|\r|\n|\t|./ig);
